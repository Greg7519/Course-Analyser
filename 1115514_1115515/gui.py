...
sources_web_scraping = [
    ...
    {"name": "Coursera",
     "url": "https://www.coursera.org/search?query=web%20development&language=English&productDifficultyLevel=Beginner&productDifficultyLevel=Intermediate&productDifficultyLevel=Advanced&productTypeDescription=Courses&topic=Computer%20Science&sortBy=BEST_MATCH"}
]
...
def add_subjects():
    ...
    flag = askyesno("Confirm Data Collection",
                    f"Are you sure you're ready to collect data with the method: {method}")  # ελέγχω αν σίγουρα θέλει να κάνει web scrape ο χρήστης
...
def auto_fetch_subjects():
    ...
    if method == "Harvard":
        source = sources_web_scraping[0]
        source_name = source["name"]
        print(f"[Auto-Scheduler] [{source_name}] Status: Starting Web Scraping Data Collection...")
        try:
            result_df = Scraper(source['url'], source['name'])
        except:
            print(f"[Auto-Scheduler] [{source_name}] Status: Failed")
            return
        current_web_scraping_source_index += 1
        ...
        source_name = "Udemy (API)"
        print(f"[Auto-Scheduler] [{source_name}] Status: Starting API Data Collection...")
        result_df = freeUdemyCourse()
    ...
    if result_df is not None and not result_df.empty:
        ...
        print(f"[Auto-Scheduler] [{source_name}] Status: Success")

        Data_df = Data_df.drop_duplicates(subset=["Title"])

        filename = "courses_1115515.csv"
        Data_df.to_csv(filename, mode="w", index=False, header=True)
        print(f"[Auto-Scheduler] [System] Status: Automatic Export to {filename} Successful")
        listbox.delete(0, tk.END)
        listbox.pack()
...
def toggle_pause():
    ...
    if scheduler_paused:
        scheduler_paused = False
        pauseBtn.config(text="Pause Αυτόματης Συλλογής")
        print("[Scheduler] Επανεκκίνηση...")
    else:
...
if __name__ == "__main__":
    ...
    pauseBtn = tk.Button(frame1, text="Pause Αυτόματης Συλλογής", width=35, command=toggle_pause)
